...
export class LexRuntime {
  ...
  static async create({ shader, planes, fp, steps, dim = 64, adapterOptions } = {}) {
    ...
    // This model's wider hidden state (96 vs. lex's 64) pushes the per-workgroup
    // scratch arrays (tA/tB/tP/red/lg) up to ~25 KiB, over the 16 KiB default
    // maxComputeWorkgroupStorageSize. Request the adapter's own higher limit
    // rather than a hard-coded number, since it varies by hardware.
    const needed = 32 * 1024;
    const device = await adapter.requestDevice(
      adapter.limits.maxComputeWorkgroupStorageSize >= needed
        ? { requiredLimits: { maxComputeWorkgroupStorageSize: needed } }
        : undefined,
    );
  ...
  #upload(data, usage, label) {
    ...
    if (totalTokens > this.#capacity.tokens) {
      ...
      // 3 words/token (12 bytes), not lex's 2 (8 bytes): lex-large's tokenizer
      // packs six extra structural fields into a third word -- see tokenizer.js.
      this.#buffers.tokens = dev.createBuffer({
        size: n * 12, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST, label: 'tokens',
      });
      ...
      // Debug-stage reads need n * dim floats, not just n classes -- sized to
      // this.dim (96 here) rather than lex's fixed 64, since a smaller cap
      // would truncate a debugStage() readback for this wider model.
      this.#buffers.staging = dev.createBuffer({
        size: n * 4 * this.dim, usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST, label: 'staging',
      });
  ...
  async #runBatch(jobs) {
    ...
    try {
      ...
      const tokenData = new Uint32Array(total * 3);
      const jobData = new Uint32Array(MAX_JOBS * (JOB_BYTES / 4));
      for (let i = 0; i < jobs.length; i++) {
        const j = jobs[i];
        tokenData.set(j.packed.subarray(0, j.count * 3), j.offset * 3);
