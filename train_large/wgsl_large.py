...
def _prefix_suffix_head(source: str) -> str:
    """Specialize the 96-wide head for the two-view v2 context tensor."""
    for index in range(6, 12):
        source = source.replace(
            f'    let gs{index} = planes[GS_P + d * 12u + {index}u];\n', '')
    source = source.replace('GS_P + d * 12u', 'GS_P + d * 6u')
    source = source.replace(
        '''        tP[j * 4u * DIM + d]             = scratch[r_stat + d];
        tP[j * 4u * DIM + DIM + d]       = scratch[r_stat + DIM + d];
        tP[j * 4u * DIM + 2u * DIM + d]  = select(0.0, scratch[r_pre + t * DIM + d], ok);
    ...
    source = source.replace('let b = j * 4u * DIM;', 'let b = j * 2u * DIM;')
    for index, offset in ((6, 192), (7, 224), (8, 256), (9, 288),
                          (10, 320), (11, 352)):
        source = source.replace(
            f'            for (var k: u32 = 0u; k < 32u; k = k + 1u) {{ acc = acc + m1(gs{index}, k, tP[b + {offset}u + k]); }}\n',
            '')
    if 'gs6' in source or 'j * 4u * DIM' in source:
        raise AssertionError('failed to specialize large prefix/suffix context')
    return source


_V1_FIELD_CASES = '''        case 10u: { return  tw2(o, i)        & 7u; }           // paren_depth
        case 11u: { return (tw2(o, i) >>  3u) & 7u; }          // brace_depth
        case 12u: { return (tw2(o, i) >>  6u) & 7u; }          // bracket_depth
        case 13u: { return (tw2(o, i) >>  9u) & 7u; }          // line_pos
        case 14u: { return (tw2(o, i) >> 12u) & 7u; }          // indent_bucket
        default:  { return (tw2(o, i) >> 15u) & 3u; }          // quote_state (f == 15)'''
_V2_FIELD_CASES = '''        case 10u: { return  tw2(o, i)        & 3u; }           // gap_prev
        case 11u: { return (tw2(o, i) >>  2u) & 3u; }          // gap_next
        case 12u: { return (tw2(o, i) >>  4u) & 7u; }          // indent
        case 13u: { return (tw2(o, i) >>  7u) & 31u; }         // line_first
        case 14u: { return (tw2(o, i) >> 12u) & 7u; }          // brace_depth
        case 15u: { return (tw2(o, i) >> 15u) & 7u; }          // paren_depth
        case 16u: { return (tw2(o, i) >> 18u) & 7u; }          // bracket_depth
        case 17u: { return (tw2(o, i) >> 21u) & 7u; }          // line_pos
        default:  { return (tw2(o, i) >> 24u) & 3u; }          // quote_state (f == 18)'''
...
def generate(meta: dict) -> str:
    ...
    head = POOL_HEAD
    prelude = PRELUDE
    if meta.get('feature_version', cfg.get('feature_version', 1)) == 2:
        prelude = prelude.replace(_V1_FIELD_CASES, _V2_FIELD_CASES)
        if _V2_FIELD_CASES not in prelude:
            raise AssertionError('failed to install large v2 feature decoder')
    if cfg.get('ctx_views', 'full') == 'prefix_suffix':
        head = _prefix_suffix_head(head)
    if t['embedding.table']['bits'] != 3:
        raise ValueError('large shader requires 3-bit embedding.table')
    embed = embed.replace(
        'wgt(EMB_P, EMB_W, EMB_B, row * EDIM + d, fp[EMB_S + f])',
        'wgt3(EMB_P, EMB_W, row * EDIM + d) * fp[EMB_S + f]')
    embed = embed.replace(
        'wgt(EMB_P, EMB_W, EMB_B,\n                                        (FLAG_ROW + b) * EDIM + d, fp[EMB_S + NFIELD])',
        'wgt3(EMB_P, EMB_W, (FLAG_ROW + b) * EDIM + d) * fp[EMB_S + NFIELD]')
    ...
    if t['embedding.up']['bits'] > 1:
        if t['embedding.up']['bits'] != 3:
            raise ValueError('large shader requires 3-bit embedding.up')
        embed = embed.replace('dotA64(up_w0, up_w1, j * EDIM)',
                              'dotQ3A(UP_P, UP_W, d * EDIM, j * EDIM, EDIM)')
    if t['head_hidden']['bits'] > 1:
        if t['head_hidden']['bits'] != 2:
            raise ValueError('large shader requires 2-bit head_hidden')
        for idx in (0, 1):
            head = head.replace(
                f'dotB192(h{idx}a, h{idx}b, h{idx}c, h{idx}d, h{idx}e, h{idx}f, j * 2u * DIM)',
                f'dotQ2B(HH_P, HH_W, o{idx} * 2u * DIM, j * 2u * DIM, 2u * DIM)')
    if t['head_out']['bits'] > 1:
        if t['head_out']['bits'] != 3:
            raise ValueError('large shader requires 3-bit head_out')
