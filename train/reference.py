...
also the readable specification of what the shader does (lex-lite, feature
version 2).
"""
...
class Reference:
    ...
    def forward(self, feats: dict[str, np.ndarray]) -> np.ndarray:
        ...
        # Prefix (declaration-gated) and suffix means. See GlobalContext.
        T = x.shape[0]
        ...
        hw = float(self.F('highway_scale')[0])
...
def features_from_code(code: str) -> dict[str, np.ndarray]:
    ...
    return tokenizer.tokens_to_arrays(toks, 2), toks
